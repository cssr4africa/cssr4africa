...
class BottomCameraTest
{
    /**
     * @brief Class to subscribe to the topic /naoqi_driver/camera/bottom/image_raw
     * 
     */

    public:
        BottomCameraTest(ros::NodeHandle &nh, image_transport::ImageTransport &it)
        {
            // Create a subscriber object to get camera raw image
            im_sub = it.subscribe("/naoqi_driver/camera/bottom/image_raw", 1, &BottomCameraTest::imageCallBack, this);
        }
...
{
    ros::init(argc, argv, "bottom_camera");
    ...
    // instanciate an object from the class BottomCameraTest

    BottomCameraTest bottom_camera(nh, it);
