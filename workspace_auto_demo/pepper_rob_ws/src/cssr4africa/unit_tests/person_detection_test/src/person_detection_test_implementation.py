...
class PersonDetectionTest:
    def __init__(self):
        """
        Initialize the PersonDetectionTest class.
    ...
    def person_data_callback(self, msg):
        """
        Process incoming person detection data containing multiple people.
        ...
        for person_id in self.person_labels:
            if person_id not in self.person_colors:
                # Generate a visually distinct color for this person
                hue = hash(person_id) % 100 / 100.0  # Use hash to get consistent color for same ID
                rgb = colorsys.hsv_to_rgb(hue, 0.8, 1.0)
                bgr = (int(rgb[2] * 255), int(rgb[1] * 255), int(rgb[0] * 255))  # Convert to BGR
                self.person_colors[person_id] = bgr
    ...
    def image_callback(self, msg):
        ...
        try:
            ...
            # Process the RGB frame with person detection overlay
    ...
    def draw_person_detection_overlay(self, image):
        """
        Draw person detection overlay on the image for multiple people.
        ...
        for i, (person_id, centroid, width, height) in enumerate(zip(
            ...
            person_width = int(width)
            person_height = int(height)
            
            # Calculate bounding box coordinates
            x1 = max(0, centroid_x - person_width // 2)
            y1 = max(0, centroid_y - person_height // 2)
            x2 = min(image.shape[1], centroid_x + person_width // 2)
            y2 = min(image.shape[0], centroid_y + person_height // 2)
            
            # Get color for this person
            person_color = self.person_colors.get(person_id, (0, 255, 0))
    ...
    def process_rgb_frame(self, cv_image):
        """
        Process an RGB frame - draw person detection overlay and save to video/image.
    ...
    def process_depth_frame(self, cv_depth):
        ...
        if (self.config.get("saveImage", False) and 
            ...
            image_path = os.path.join(
                self.unit_test_package_path, 
                'person_detection_test/data', 
                f'person_detection_test_depth_image_{timestamp}.png'
            )
