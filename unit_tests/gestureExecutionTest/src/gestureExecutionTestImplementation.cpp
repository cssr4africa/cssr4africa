...
TEST_F(GestureExecutionUnitTest, TestIconicGestures)
{
    ...
    for (int i = 0; i < iconic_gesture_IDs.size(); i++) {
        ...
        if (interface == "service") {
            ...
            command = "rostopic pub --once /gestureExecution/perform_gesture/goal cssr_system/gestureActionGoal \"header: {seq: 0, stamp: {secs: 0, nsecs: 0}, frame_id: ''} goal_id: {stamp: {secs: 0, nsecs: 0}, id: ''} goal: {gesture_type: 'iconic', gesture_id: " + gesture_ID + ", arm: '', deictic_shape: '', gesture_duration: 3000, bow_nod_angle: 45, location_x: -1.2, location_y: 3.0, location_z: 0.82}\"";
        }
...
TEST_F(GestureExecutionUnitTest, TestDiecticGestures)
{
    ...
    for (int i = 0; i < point_coordinates.size(); i++) {
        ...
        if (interface == "service") {
            ...
            command = "rostopic pub --once /gestureExecution/perform_gesture/goal cssr_system/gestureActionGoal \"header: {seq: 0, stamp: {secs: 0, nsecs: 0}, frame_id: ''} goal_id: {stamp: {secs: 0, nsecs: 0}, id: ''} goal: {gesture_type: 'deictic', gesture_id: 02, arm: '" + arm + "', deictic_shape: '" + deicticShape + "', gesture_duration: 3000, bow_nod_angle: 45, location_x: " + point_x + ", location_y: " + point_y + ", location_z: " + point_z + "}\"";
        }
...
TEST_F(GestureExecutionUnitTest, TestBowGestures)
{
    ...
    for (int i = 0; i < bow_angles.size(); i++) {
        ...
        if (interface == "service") {
            ...
            command = "rostopic pub --once /gestureExecution/perform_gesture/goal cssr_system/gestureActionGoal \"header: {seq: 0, stamp: {secs: 0, nsecs: 0}, frame_id: ''} goal_id: {stamp: {secs: 0, nsecs: 0}, id: ''} goal: {gesture_type: 'bow', gesture_id: 02, arm: '', deictic_shape: '', gesture_duration: 3000, bow_nod_angle: " + bow_angle + ", location_x: 0, location_y: 0, location_z: 0}\"";
        }
        output = invoke_command(command.c_str());
        ros::Duration(2).sleep();
...
TEST_F(GestureExecutionUnitTest, TestNodGestures)
{
    ...
    for (int i = 0; i < nod_angles.size(); i++) {
        ...
        if (interface == "service") {
            ...
            command = "rostopic pub --once /gestureExecution/perform_gesture/goal cssr_system/gestureActionGoal \"header: {seq: 0, stamp: {secs: 0, nsecs: 0}, frame_id: ''} goal_id: {stamp: {secs: 0, nsecs: 0}, id: ''} goal: {gesture_type: 'bow', gesture_id: 02, arm: '', deictic_shape: '', gesture_duration: 3000, bow_nod_angle: " + nod_angle + ", location_x: 0, location_y: 0, location_z: 0}\"";
        }
        output = invoke_command(command.c_str());
        ros::Duration(2).sleep();
