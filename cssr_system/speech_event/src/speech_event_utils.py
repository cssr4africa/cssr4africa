...
def get_model(model_name, rw_model_path, en_model_path, lang, device):
    """ Get a speech recognition model object

    Parameters:
        model_name (str):               one of conformer-transducer, parakeet, whisper
        rw_model_path (str):            path to Kinyarwanda model matching the passed model name
        en_model_path (str):            path to English model matching the passed model name
        lang (str):                     one of kinyarwanda, english
        device (str | torch.device):    device to load the model to, gpu or cuda

    Returns:
        model:  speech recognition model object
    """
...
def get_hypotheses(model_name, model, audio_samples):
    """ Trancribe speech contained in an audio signal

    Parameters:
        model_name (str):               one of conformer-transducer, parakeet, whisper
        model (object):                 loaded speech recognition model object
        audio_samples (torch.Tensor):   audio tensor to transcribe
...
def get_transcription(model_name, hypotheses, text_for_unrecognised, conf_threshold):
    ...
    Parameters:
        model_name (str):               one of conformer-transducer, parakeet, whisper
        hypotheses (dict):              dictinary of transcription results
        text_for_unrecognised (str):    text to use if the transcribed text falls below conf_threshold
        conf_threshold (float):         the confidence threshold above which a transcription is assumed to be correct

    Returns:
        tuple[str, str]:    transcribed text, log message to print to the terminal
    """
