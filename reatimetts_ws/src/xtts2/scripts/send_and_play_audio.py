...
if len(sys.argv) < 2:
    print "Usage: send_and_play_audio.py <audio_file_path> [robot_ip]"
    sys.exit(1)

audio_path = sys.argv[1]
audio_name = audio_path.split("/")[-1]

# Get IP from args or env or default
IP = sys.argv[2] if len(sys.argv) > 2 else os.environ.get("NAO_IP", "172.29.111.230")
PORT = 9559

print "Target Robot IP:", IP
